...
html_doc.css('#TPASection_jsrook8y > div > div > div > div > section > div > ul > li').first(3).each do |element|
  costume = Costume.new(name: Faker::Coffee.blend_name, description: element.search('._3RqKm > h3').text, price: element.search('._23ArP').text.to_i, photo: element.search("div[style*='background-image']")[0].to_s[/(https?:\/\/.*\.(?:png|jpg))/i])
